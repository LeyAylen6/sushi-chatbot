...
const chat = async (req, res, next) => {
    ...
    const prompt = `Eres un asistente amable de un restaurante de sushi. Responde siguiendo estas reglas:
        ...
        - Nunca le hagas, ni le respondas con preguntas al usuario que se respondan con SI o NO.
        - Responde en JSON SOLO con los campos \`response\`, \`_id\`, \`price\`, \`client\`, y \`products\`, No puedes agregar ninguno extra:
        {
    ...
    try {
        ...
        if (cohereResponseJson?.products?.length && cohereResponseJson?.client) {
            ...
            cohereResponseJson.response += ` Su numero de pedido es: ${newOrder._id}, con su numero de pedido puede socitar agregar mas productos al mismo pedido de ser necesario.`
